...
import { RANKS, RULES } from '../../lib/cod.js'
  ...
  <section class="pt-6 space-y-6">
    ...
    <details class="rounded-2xl border-2 border-line bg-night-soft p-4">
      ...
      <ul class="mt-3 space-y-2 text-sm text-foam/80">
        <li v-for="rank in RANKS" :key="rank" class="flex gap-3">
          ...
          <span><strong>{{ RULES[rank].title }}</strong> — {{ RULES[rank].text }}</span>
        </li>
      </ul>
    </details>
    ...
    <div class="space-y-2">
      <button
        ...
        :disabled="!canStart"
        @click="act(() => c.startGame())"
      >
        Schud de kaarten →
      </button>
      <p v-if="!canStart" class="text-center text-xs text-foam/50">
        Minstens 2 spelers nodig — je hebt er nog {{ 2 - c.state.players.length }} te kort.
      </p>
    </div>
  </section>
</template>
